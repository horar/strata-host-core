...
QString ConfigFileSettings::logLevel() const
{
    ...
    if (logLevels.contains(level)) {
        qCDebug(lcLogConf) << "Current log level :" << level;
        return level;
    } else {
        qCWarning(lcLogConf) << "Parameter" << LOG_LEVEL_SETTING << "is set to" << level << ", which is not a valid value.";
...
int ConfigFileSettings::maxFileSize() const
{
    ...
    if (ok) {
        if (logMaxFileSize >= MIN_LOGFILE_SIZE && logMaxFileSize <= MAX_LOGFILE_SIZE) {
            qCDebug(lcLogConf) << "Current max size of log file :" << logMaxFileSize;
            return logMaxFileSize;
        } else {
            QString fileSizeString = settings_->value(LOG_MAXSIZE_SETTING).toString();
            qCWarning(lcLogConf) << "Max file size out of range :" << fileSizeString;
            emit corruptedFile(LOG_MAXSIZE_SETTING, fileSizeString);
        }
    } else {
        QString fileSizeString = settings_->value(LOG_MAXSIZE_SETTING).toString();
        qCWarning(lcLogConf) << "Parameter" << LOG_MAXSIZE_SETTING << "is set to" << fileSizeString << ", which is not a valid value.";
        emit corruptedFile(LOG_MAXSIZE_SETTING, fileSizeString);
    }
    return -1;
}
...
int ConfigFileSettings::maxNoFiles() const
{
    ...
    if (ok) {
        if (logMaxNoFiles >= MIN_NOFILES && logMaxNoFiles <= MAX_NOFILES) {
            qCDebug(lcLogConf) << "Current max size of log file :" << logMaxNoFiles;
            return logMaxNoFiles;
        } else {
            QString noFilesString = settings_->value(LOG_MAXNOFILES_SETTING).toString();
            qCWarning(lcLogConf) << "Max number of files out of range :" << noFilesString;
            emit corruptedFile(LOG_MAXNOFILES_SETTING, noFilesString);
        }
    } else {
        QString noFilesString = settings_->value(LOG_MAXNOFILES_SETTING).toString();
        qCWarning(lcLogConf) << "Parameter" << LOG_MAXNOFILES_SETTING << "is set to" << noFilesString << ", which is not a valid value.";
        emit corruptedFile(LOG_MAXNOFILES_SETTING, noFilesString);
    }
    return -1;
}
...
QString ConfigFileSettings::qtFilterRules() const
{
    ...
    if (qtFilterRules.isEmpty() == false) {
        qCDebug(lcLogConf) << "Current Qt filter rules :" << qtFilterRules;
        return qtFilterRules;
    } else {
        qCWarning(lcLogConf) << "Parameter" << LOG_FILTERRULES_SETTING << "is set to" << qtFilterRules << ", which is not a valid value.";
...
QString ConfigFileSettings::qtMsgPattern() const
{
    ...
    if (qtMsgPattern.isEmpty() == false) {
        qCDebug(lcLogConf) << "Current Qt message pattern :" << qtMsgPattern;
        return qtMsgPattern;
    } else {
        qCWarning(lcLogConf) << "Parameter" << LOG_QT_MSGPATTERN_SETTING << "is set to" << qtMsgPattern << ", which is not a valid value.";
...
QString ConfigFileSettings::spdlogMsgPattern() const
{
    ...
    if (spdMsgPattern.isEmpty() == false) {
        qCDebug(lcLogConf) << "Current spdlog message pattern :" << spdMsgPattern;
        return spdMsgPattern;
    } else {
        qCWarning(lcLogConf) << "Parameter" << LOG_SPD_MSGPATTERN_SETTING << "is set to" << spdMsgPattern << ", which is not a valid value.";
