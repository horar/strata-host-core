...
bool SDSModel::startHcs()
{
    ...
    if (QFile::exists(hcsPath)) {
        ...
        qCDebug(logCategoryDevStudio) << "Starting HCS:" << hcsPath << "(" << hcsConfigPath << "), identifier:" << hcsIdentifier_;

        hcsProcess_->start(hcsPath, arguments, QIODevice::ReadWrite);
        if (hcsProcess_->waitForStarted() == false) {
            qCWarning(logCategoryDevStudio) << "Process does not started yet (state:" << hcsProcess_->state() << ")";
            return false;
        }
    } else {
        qCCritical(logCategoryDevStudio) << "Failed to start HCS: does not exist";
        return false;
    }
...
bool SDSModel::killHcs()
{
    ...
    if (hcsProcess_->state() == QProcess::Running) {
        qCDebug(logCategoryDevStudio) << "waiting for HCS gracefull finish...";
        if (hcsProcess_->waitForFinished(5000) == true) {
            return true;
        }

#ifdef Q_OS_UNIX
        qCDebug(logCategoryDevStudio) << "terminating HCS...";
        hcsProcess_->terminate();
        QThread::msleep(100);   //This needs to be here, otherwise 'waitForFinished' waits until timeout
        if (hcsProcess_->waitForFinished(5000) == true) {
            return true;
        }
        qCWarning(logCategoryDevStudio) << "Failed to terminate the server";
#endif

        qCDebug(logCategoryDevStudio) << "killing HCS...";
        hcsProcess_->kill();
        if (hcsProcess_->waitForFinished(5000) == true) {
            return true;
        }
        qCCritical(logCategoryDevStudio) << "Failed to kill HCS server";
        return false;
    }
...
{
    qCInfo(logCategoryDevStudio) << "HCS started";
...
void SDSModel::finishHcsProcess(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (exitStatus == QProcess::NormalExit && exitCode == (EXIT_FAILURE + 1)) {
        ...
        qCDebug(logCategoryDevStudio) << "Quitting - another HCS instance was running";
        return;
    }
...
{
    qCDebug(logCategoryDevStudio) << error << hcsProcess_->errorString();
}
...
QString SDSModel::openLogViewer()
{
    ...
    if (logViewerInfo.exists() == false) {
        qCCritical(logCategoryDevStudio) << "Log Viewer at location " + logViewerPath + " does not exist.";
        return "Log Viewer not found.";
    }
    if (logViewerInfo.isExecutable() == false) {
        qCCritical(logCategoryDevStudio) << "Log Viewer at location " + logViewerPath + " is not executable file.";
    ...
    if (QProcess::startDetached(logViewerPath, {sdsLog, hcsLog}) == false) {
        qCCritical(logCategoryDevStudio) << "Log Viewer from location " + logViewerPath + " with log files " + sdsLog + " and " + hcsLog + " failed to start.";
