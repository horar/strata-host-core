...
echo Pools Default
out=$(curl -s -w "%{http_code}\n" -X POST -u sync_gateway:sync_gateway $COUCHBASE_ENDPOINT/pools/default -d 'clusterName=strata&memoryQuota=312&indexMemoryQuota=512&ftsMemoryQuota=256')
...
echo Create bucket
out=$(curl -s -w "%{http_code}\n" -X POST -u sync_gateway:sync_gateway $COUCHBASE_ENDPOINT/pools/default/buckets -d name=strata -d ramQuotaMB=100 -d authType=none)
