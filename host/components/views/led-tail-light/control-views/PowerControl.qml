...
Item {
    ...
    Component.onCompleted: {
        ...
        Help.registerTarget(boostOCP, "Indicates the VLED input voltage source has faulted. \n • Boost fault = the boost converted may have reached its maximum input current of 400mA - this is a limitation of the current provided from the USB cable. Use Buck or Bypass VLED Input Voltage Type with external power supply for higher current applications.\n • Buck fault = the buck converter may have reached its 1A current limit. Remove the load and the buck converter will restart automatically.\n • Bypass = the bypass load switch may have reached its 1A current limit. Remove the load and the bypass load switch will restart automatically.", 2, "powerControlHelp")
