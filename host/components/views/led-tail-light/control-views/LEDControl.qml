...
Item {
    ...
    Component.onCompleted: {
        ...
        Help.registerTarget(partNumber, "Indication of LED driver part assembled on PCB. The part number is reported to the user interface based on state of pin PD9, low = NCV7684, high = NCV7685.", 13, "ledDriverHelp")
        Help.registerTarget(filterHelpContainer1, "Toggles each LED on or off. If PWM is enabled, these controls will be disabled.", 0, "ledDriverHelp")
        Help.registerTarget(filterHelpContainer2, "Toggles each LED with to use onboard LEDs or externally connected LEDs through 24 pin header at the top of the PCB.", 1, "ledDriverHelp")
        Help.registerTarget(filterHelpContainer3, "Toggles PWM for each LED channel on or off. If PWM is disabled, these controls will also be disabled." , 2, "ledDriverHelp")
        Help.registerTarget(filterHelpContainer4, "Indicates fault on each LED channel. I2C Open Load Diagnostic must be set to “Diagnostic Only” or “Auto Retry” and DIAGEN above 2V for fault status to update." , 3, "ledDriverHelp")
        Help.registerTarget(filterHelpContainer5, "Sets the duty cycle of the PWM for each LED channel with 128 step resolution. Maximum and minimum duty cycles will switch LED fully on and off respectively. If PWM is disabled, these controls will also be disabled.", 4, "ledDriverHelp")
        ...
        Help.registerTarget(scIset,"Set when a short-circuit on the external resistor on ISET pin. Permanently latches after 10µs.",5,"ledDriverHelp")
        Help.registerTarget(i2Cerr,"Set if an error has been detected during the I2C communication. Reset on register read.",6,"ledDriverHelp")
        Help.registerTarget(uv,"Set if under voltage condition when VS is below VSUV (4.1V typical). All channels will be turned off.",7,"ledDriverHelp")
        Help.registerTarget(diagRange,"Set when divided voltage is above the VDiagenTH threshold and reset when below. Default voltage threshold is fixed on this PCB to 9V. User can adjust this threshold with the onboard potentiometer by removing lower divider resistor and shorting jumper.",8,"ledDriverHelp")


        Help.registerTarget(tw,"Thermal warning that is set when junction temperature is above the Tjwar_on threshold (140°C typical) and reset on register read and when temperature is below Tjwar_on minus Tjsd_hys threshold (127.5°C typical).",9,"ledDriverHelp")
        Help.registerTarget(tsd,"Thermal shutdown set when junction temperature is above the TSD threshold (170°C typical) and reset on register read and when temperature is below TSD minus Tjsd_hys threshold (157.5°C typical).",10,"ledDriverHelp")

         Help.registerTarget(diagerr,"Set when DIAG pin is forced low externally. Permanently latches after 10µs.",11,"ledDriverHelp")
         Help.registerTarget(ol,"Set when at least one LED channel is in an open load. Permanently latches after 10µs. I2C Open Load Diagnostic must be set to “Diagnostic Only” or “Auto Retry” and DIAGEN above 2V for OL to update.",12,"ledDriverHelp")
