...
Item {
    ...
    Component.onCompleted: {
        ...
        • The CRC polynomial is 0x2F (x⁸+x⁵+x³+x²+x+1)
        • The initial XOR is 0xFF
        • The final XOR is 0x00
        • Reflect and data and remainder are disabled \n
This design will by default search for a device with I2C CRC disabled and 7-bit I2C slave address of 0x60 – if no LED driver is found the user must enter a valid I2C configuration to enable the user interface.", 0, "samoptHelp")
        Help.registerTarget(vDDVoltageDisconnect, "Removes the VDD supply to the LED driver. This will put the LED driver into SAM \n– similar to changing the Mode control to SAM. A popup will appear that requires the user to reconnect VDD as the LED driver will not respond to I2C communication after disconnecting VDD. The previously set LED driver Mode will be enabled.", 5, "samoptHelp")
        Help.registerTarget( i2cStandalone, "Controls the mode of the LED driver to be controlled by I2C or enter SAM. I2C mode adheres to the LED register settings configured on the LED Control tab. SAM mode adheres to the settings on this tab from the SAM_CONF_1/2 controls.", 3, "samoptHelp")
        ...
        • Diagnostic Only = During open load fault, the DIAG pin is pulled low with no change to current regulation. If fault is recovered DIAG is released.", 9, "samoptHelp")
        ...
        Help.registerTarget(current7bit, "The current 7-bit I2C slave address used to communicate to the LED driver.", 1, "samoptHelp")
        Help.registerTarget(filterHelpContainer2, "Set a new 7-bit I2C slave address. Click Apply I2C Address to change. The value entered will be coerced to a valid I2C slave address between 0x60 and 0x7F. The Current 7-bit I2C Address will be updated with the new address if successfully applied. This design will by default search for a device with I2C CRC disabled and 7-bit I2C slave address of 0x60 - if no LED driver is found the user must enter a valid I2C configuration to enable the user interface.", 2, "samoptHelp")
    ...
    Item {
        ...
        y: topLeft.y
        function update() {
            topLeft = out1.mapToItem(root, 0,  0)
            bottomRight = samOut12.mapToItem(root, samOut12.width, samOut12.height)
        }
    }
