...
Rectangle {
    ...
    property int gaugeTitleSize: 12
...
//                color: styleData.value > gauge.value ? root.gaugeRearColor : (styleData.value > gauge.value-1 ? "red" : "root.gaugeFrontColor")
        ...
        Text {
            ...
            font.pixelSize: Math.min(gauge.width/ 5, gauge.width/Math.max((root.maximumValue+ "").length, (root.minimumValue + "").length)) // Scale the gauge font based on what the largest or smallest number that might be displayed
