...
#define DEBUG(...) printf("\n    TEST Database: "); printf(__VA_ARGS__); printf("\n");
...
/*
    ...
    // Check DB is open
    if (!DB_1.isOpen()) {
        DEBUG("DB 1 is not open.");
    } else {
        DEBUG("DB 1 is open.");
    }

    // Create a document "Doc_1" on DB 1
    CouchbaseDocument Doc_1(&DB_1, "Doc_1");
    ...
    std::string modify_key = "age";
    Doc_1.set(modify_key, 123);

    // Save document to DB again
    DB_1.save(&Doc_1);
    ...
    CouchbaseDocument Doc_2(&DB_1, "Doc_2");
